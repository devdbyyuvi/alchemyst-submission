...
export default function ChatWindow() {
  ...
  useEffect(() => {
    if (activeChatBlockId && chatContainerRef.current) {
      const element = chatContainerRef.current.querySelector(
        `[data-block-id="${activeChatBlockId}"]`,
      );
      if (element) {
    ...
    <div className="flex flex-col h-full bg-gray-900 text-white relative">
      ...
      >
        {blocks.length === 0 && (
          <div className="flex-1 flex items-center justify-center text-gray-500">
            Send a message. Try:{" "}
            {`"summarize the report" or "show me the full database schema"`}
          </div>
        )}
        {blocks.map((block) => {
          if (block.type === "user")
            return (
              <div
                key={block.id}
                className="bg-blue-600 p-3 rounded-lg self-end max-w-md shadow-lg"
              >
                {block.text}
              </div>
            );
          if (block.type === "text")
            return (
              <div
                key={block.id}
                className="bg-gray-800 p-3 rounded-lg self-start max-w-2xl whitespace-pre-wrap shadow-lg"
              >
                {block.text}
          ...
          if (block.type === "tool_call")
            return (
              <div
                key={block.id}
                data-block-id={block.call_id}
                onClick={() =>
                  useUIStore.getState().setActiveTimelineEventSeq(block.seq)
                }
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="text-[10px] text-gray-500">
                    {block.status === "pending" ? "Waiting..." : "Done"}
                  </div>
                </div>
      ...
      <div className="p-4 border-t border-gray-700 bg-gray-950">
        <div className="flex gap-2 max-w-4xl mx-auto">
          ...
          <button
            onClick={handleSend}
            className="bg-blue-600 px-6 py-3 rounded-lg hover:bg-blue-700 font-bold transition-colors"
          >
            Send
          </button>
        </div>
